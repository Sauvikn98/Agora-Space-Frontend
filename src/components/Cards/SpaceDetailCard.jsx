...
function SpaceDetailCard() {
    ...
    return (
        <div className='w-full '>
            ...
            <div key={space.id} className=" bg-gray-300">
                <div onClick={() => handleNavigate(posts.id)} className="p-6 pb-20 lg:ml-7 mr-7 lg:mt-0  space-y-4">
                    <div className='mb-10'>
                        <h3 className="text-lg font-bold text-black">{space.name}</h3>
                        <p className="text-black">{space.description}</p>
                    </div>
                    <AllSpacePost spaceId={space._id} />
                </div>
            </div>
        </div>
    )
