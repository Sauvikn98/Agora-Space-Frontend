        ...
        const getPostsForSpaces = async () => {
            try {
                const promises = spaces.map(space =>
                    axios.get(`${API_SPACES_GET_POSTS}/${space._id}/posts`)
                        .then(response => response.data)
                );
                const results = await Promise.all(promises);
                const posts = results.flat();
                const LatestPostsBySpace = posts.reduce((acc, post) => {
                    const spaceId = post.space;
                    if (!acc[spaceId] || post.createdAt > acc[spaceId].createdAt) {
                        acc[spaceId] = post;
                    }
                    return acc;
                }, {});
